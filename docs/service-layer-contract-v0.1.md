...
- Initial participant refund request/review/proof-upload RPC/API drafts that restrict requests to owned confirmed orders, restrict review and proof upload to refund managers, update registration/payment/refund state, store refund proof metadata after private Storage object verification, and write `audit_logs`.
...
- End-to-end real Supabase session/RLS validation for refund requests, review, and proof upload.
