...
- 一个用户可以创建多个活动。
- 一个活动有一个创建者，但可以绑定多个组织者。
- 活动组织者可以拥有不同角色，例如主办、联合主办、财务、现场协作和只读。
- 一个活动可以有多条报名。
- 一个活动有一组财务设置。
- 一个活动可以记录多条支出。
- 一条报名属于一个活动和一个主报名用户。
...
- `id` 是数据库内部 ID，不展示给普通用户作为主要识别方式。
- `public_code` 是活动公开 ID，用于搜索、分享、现场核对和客服查询。
- `public_code` 在平台内唯一，建议格式为 `GU-{eventCode}-{date}` 或组织者自定义前缀。
- 活动创建者默认写入 `organizer_id`，同时应写入 `event_organizers` 并标记为主办。
- 被写入 `event_organizers` 且具备管理权限的用户可以进入活动管理台。
- 活动发布后才可以被参与者报名。
- `public` 活动进入活动广场。
...
- `owner`：主办，拥有完整管理权限。
- `co_host`：联合主办，可以编辑活动和处理报名。
- `finance`：财务，可以查看和处理付款、支出和财务汇总。
- `checkin`：现场协作，可以查看名单和处理签到。
- `viewer`：只读，可以查看活动数据但不能修改。

### 规则

- 同一个用户在同一活动中只能有一条组织者记录。
- 至少需要保留一个 `owner`。
- 删除或降级最后一个 `owner` 前必须先转移主办权限。
- 权限判断不能只依赖前端展示，必须在数据库 RLS 或后端服务层校验。
- 活动组织者的 GatherUp ID 可以展示给参与者，用于增强信任和后续关注组织者。
...
- `free`：免费活动，只记录支出。
- `paid`：收费活动，收入来自报名订单和付款确认。
- `split`：AA 记账活动，先记录支出，活动后按参与人数分摊。
...
- 收入不应手动乱填，v0.1 先从报名订单和付款状态汇总。
- 支出由有财务权限的组织者手动添加。
- 每条支出应支持凭证截图或发票附件。
- 活动财务页应展示确认收入、待审核收入、总支出、预计结余和人均成本。
- 财务权限不等于完整活动编辑权限，后续应在 RLS 或服务层继续细分。
