...
Current status: frontend prototype plus commercial v0.1 backend foundation, contract tests, organizer workflow UX improvements, clean Supabase dev/staging execution validation, Supabase SSR middleware auth, Bearer/SSR-cookie API auth, atomic registration order creation, private Storage-backed proof flows, audited payment/check-in/refund RPC scaffolding, and opt-in real Supabase RPC integration tests in progress.

The codebase already includes the Next.js app skeleton, local/mock workflows, Supabase Auth/profile adapter preparation, Supabase SSR route protection middleware, Bearer/SSR-cookie API auth helpers, commercial schema/seed/Storage SQL drafts, auth/schema/storage/API contract tests, event creation prototype, organizer workspace, organizer workflow stepper, dynamic next-action guidance, registration flow, atomic registration RPC integration, JWT-protected organizer APIs, private payment-proof Storage upload scaffolding, audited payment-review RPC scaffolding, participant refund request/review/proof-upload RPC scaffolding, seat-lock RPC/API scaffolding, an initial real order-detail seat selection panel, audited check-in RPC scaffolding, seat management prototype, finance prototype, notification center, venue intelligence prototype, architecture briefing docs, committed README screenshots, and real Supabase validation runbooks/logs.

The original live Supabase project has been restored and audited. It is partially initialized, not empty and not complete, so the full schema draft must not be run against it as-is. A clean dev/staging Supabase project has executed `schema.sql`, `seed.sql`, and corrected `storage.sql`; remaining post-execution validation queries still need to be recorded. The next engineering phase is to run the opt-in RPC integration suite against the clean Supabase project, validate the private Storage proof paths with real sessions, validate seat-lock concurrency, expand transactional service functions, replace prototype data with real Supabase-backed workflows, add organizer verification, and build minimum admin review.

## One-paragraph README Summary

GatherUp is an offline event operations platform focused first on community/fandom activities such as screenings, birthday cafes, and small fan gatherings. It helps organizers manage activity creation, registration, organizer-collected payment proof review, refunds, seat selection, notifications, lightweight check-in, finance, venue knowledge, and exports in one place. The current repository contains the frontend prototype, commercial v0.1 product decisions, Supabase schema/Storage drafts, atomic registration and audited workflow RPCs, JWT/SSR-cookie-gated APIs, private Storage-backed proof submission paths, static contract tests, opt-in real Supabase RPC integration tests, committed product screenshots, and validation notes for moving from mock data to real Supabase-backed workflows.
