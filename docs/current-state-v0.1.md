...
- 继续补齐活动创建后的协作者邀请确认、更细的活动审核队列、发布状态流和发布后编辑约束；当前已支持基础信息编辑、协作者添加/调整/移除、事务化审计日志记录、主办台审计时间线和收费活动发布前认证门槛，并支持从草稿/数调/待开放报名推进到 `registration_open`。
...
- 实现组织者认证申请/审核 UI、活动审核、投诉/争议和最小后台；当前收费活动发布 API 已先接入组织者认证状态门槛。
