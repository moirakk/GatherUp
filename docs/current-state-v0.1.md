...
当前是 v0.1 前端原型加商业化后端基础接入阶段，并已经开始从 mock 数据迁移到真实 Supabase 读取和受控写入。商业化 v0.1 的关键产品决策、schema/seed/Storage 草案、服务层契约、静态契约测试、真实 Supabase 只读预检/覆盖率审计、干净 dev/staging Supabase 执行验证、公开活动读取适配、JWT 主办 API 鉴权、原子报名 RPC 调用、首条付款凭证 Storage 上传链路、付款审核 RPC 草案和座位锁 RPC 草案已经推进完成一轮。

当前仓库应按两层理解：

- 已实现层：Next.js 前端原型、mock/local 数据流程、公开活动 Supabase 读取适配、Supabase 账号适配、JWT 保护的主办 API、原子报名订单创建、初始付款凭证 Storage 上传链路、商业化 SQL/Storage 草案、文档源头和静态契约测试。
...
- `/api/orders` 已改为用户 JWT client 调用 RPC；前端在 Supabase 登录态下会附带 access token，让数据库内的 `auth.uid()` 和 `current_app_user_id()` 可用。
...
- 退款申请已新增 `request_refund_atomic` RPC 草案和 `/api/orders/refund`：参与者只能为自己的已确认订单申请退款，数据库函数内创建 `refund_requests`，同步订单/付款为退款中，并写入 `audit_logs`。退款审核、退款凭证上传、参与者确认和争议处理仍需继续补齐。
- 主办敏感 API 已从原型 cookie 身份切换到 Supabase Bearer token 验证：活动创建、付款审核、核销、名单导出和财务导出不再信任可被客户端伪造的 `gatherup_id` cookie。
- 这些 API/RPC 仍是早期集成层：付款凭证 Storage 链路、付款审核 RPC、座位锁 RPC、核销 RPC 和退款申请 RPC 还需要在干净 Supabase 项目中用真实用户 session 做端到端验证；waitlist 和完整 RLS 行为实测仍需要继续补齐。
