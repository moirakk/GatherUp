...
当前是 v0.1 前端原型加商业化后端基础接入阶段，并已经开始从 mock 数据迁移到真实 Supabase 读取和受控写入。商业化 v0.1 的关键产品决策、schema/seed/Storage 草案、服务层契约、静态契约测试、真实 Supabase 只读预检/覆盖率审计、干净 dev/staging Supabase 执行验证、公开活动读取适配、JWT 主办 API 鉴权、原子报名 RPC 调用、首条付款凭证 Storage 上传链路和付款审核 RPC 草案已经推进完成一轮。
...
- 目标层：商业化 v0.1，需要真实 Supabase 执行、服务层事务、Storage 上传、权限实测、退款、签到、后台和完整审计日志写入。
...
- 这些 API 仍是早期集成层：付款凭证 Storage 链路和付款审核 RPC 还需要在干净 Supabase 项目中用真实用户 session 做端到端验证；正式 seat lock/assignment、waitlist 和完整 RLS 行为实测仍需要继续补齐。
