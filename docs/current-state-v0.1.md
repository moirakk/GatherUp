...
- 组织工作台已新增主办认证申请入口：登录用户可以读取自己的 `organizer_verifications` 状态，并通过 `/api/organizer/verification` 提交或更新待审核资料；已通过或暂停状态会被 API 阻止重复提交。管理员审核 UI 仍待补齐。
...
- 实现组织者认证审核 UI、活动审核、投诉/争议和最小后台；当前主办工作台已支持认证申请，收费活动发布 API 已接入组织者认证状态门槛。
