...
当前是 v0.1 前端原型加商业化后端基础草案阶段，并已经开始从 mock 数据迁移到真实 Supabase 读取。商业化 v0.1 的关键产品决策、schema/seed/Storage 草案、服务层契约、静态契约测试、真实 Supabase 只读预检/覆盖率审计、干净 dev/staging Supabase 执行验证，以及公开活动读取适配已经推进完成一轮。

当前仓库应按两层理解：

- 已实现层：Next.js 前端原型、mock/local 数据流程、公开活动 Supabase 读取适配、Supabase 账号适配、商业化 SQL/Storage 草案、文档源头和静态契约测试。
