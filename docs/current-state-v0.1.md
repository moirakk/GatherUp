...
- 目标层：商业化 v0.1，需要真实 Supabase 执行、服务层事务、Storage 上传、权限实测、退款、后台和完整审计日志写入。
...
- 这些 API/RPC 仍是早期集成层：付款凭证 Storage 链路、付款审核 RPC、座位锁 RPC 和核销 RPC 还需要在干净 Supabase 项目中用真实用户 session 做端到端验证；waitlist 和完整 RLS 行为实测仍需要继续补齐。
