...
- Organizer event consoles can now add, remove, or adjust non-owner collaborators by GatherUp ID through an authenticated edit-permission API route. The route verifies the current user's event edit permission first, performs controlled user lookup and `event_organizers` writes, and records collaborator changes in `audit_logs`.
...
- Broader transactional service functions, email business notifications, organizer verification UI, admin review UI, collaborator invite-acceptance and RPC-transaction audit hardening, richer event review gates, expense proof editing/voiding, venue review flows, complaints, and data retention jobs are still planned.
...
2. Expand the real data service layer beyond the current event creation, basic event editing, collaborator management, and open-registration baseline: invite acceptance, collaborator-management RPCs, visibility, review gates, and richer post-publish edit constraints.
