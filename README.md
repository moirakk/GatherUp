...
This requires `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, and `SUPABASE_SERVICE_ROLE_KEY`. The test creates isolated temporary Supabase Auth users and events, validates `create_registration_atomic`, then cleans up.
...
2. Execute `GATHERUP_RUN_RPC_INTEGRATION=1 npm run test:integration:rpc` against clean Supabase to validate `create_registration_atomic` normal, duplicate, unauthenticated, and concurrent capacity behavior.
