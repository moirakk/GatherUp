...
- Seat locking now has PostgreSQL RPC drafts plus JWT API entry points for expiring stale locks, creating active locks, and confirming assignments under database constraints. UI integration and live Supabase validation are still pending.
...
8. Wire seat-lock RPCs into the participant seat selection UI and validate concurrency behavior.
9. Lightweight check-in, refund tracking, notifications, export jobs, complaints, and minimum admin backend.
