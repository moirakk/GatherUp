...
describe("commercial schema contract", () => {
  ...
  it("keeps refund requests atomic and audited", () => {
    expectSql(schema, "create or replace function public.request_refund_atomic(");
    expectSql(schema, "v_user_id := public.current_app_user_id();");
    expectSql(schema, "from public.registrations r");
    expectSql(schema, "left join public.payments p on p.registration_id = r.id");
    expectSql(schema, "for update of r, p;");
    expectSql(schema, "v_order.user_id <> v_user_id");
    expectSql(schema, "v_order.registration_status not in ('confirmed', 'refunding')");
    expectSql(schema, "insert into public.refund_requests");
    expectSql(schema, "update public.registrations");
  ...
  it("keeps refund review atomic and audited", () => {
    ...
    expectSql(schema, "insert into public.audit_logs");
    expectSql(schema, "insert into public.notification_deliveries");
    expectSql(schema, "'workflow', 'refund_review'");
    expectSql(schema, "case when p_decision = 'APPROVED' then 'refund_approved' else 'refund_rejected' end");
    expectSql(schema, "case when p_decision = 'APPROVED' then 'Refund approved' else 'Refund rejected' end");
  ...
  it("keeps refund proof upload atomic and audited", () => {
    expectSql(schema, "create or replace function public.record_refund_proof_atomic(");
    expectSql(schema, "v_actor_id := public.current_app_user_id();");
    expectSql(schema, "from public.refund_requests rr");
    expectSql(schema, "join public.registrations r on r.id = rr.registration_id");
    ...
    expectSql(schema, "status = 'proof_uploaded'");
    expectSql(schema, "paid_at = v_now");
    expectSql(schema, "update public.registrations");
    expectSql(schema, "update public.payments");
    expectSql(schema, "insert into public.audit_logs");
    expectSql(schema, "insert into public.notification_deliveries");
  ...
  it("keeps refund dispute resolution atomic and audited", () => {
    expectSql(schema, "create or replace function public.resolve_refund_dispute_atomic(");
    expectSql(schema, "v_resolution not in ('CONFIRM_REFUNDED', 'REOPEN_PROOF')");
    expectSql(schema, "from public.refund_requests rr");
    expectSql(schema, "join public.registrations r on r.id = rr.registration_id");
    expectSql(schema, "join public.payments p on p.id = rr.payment_id");
    expectSql(schema, "for update of rr, r, p;");
    expectSql(schema, "public.can_handle_event_refunds(v_refund.event_id)");
    expectSql(schema, "v_refund.refund_status <> 'disputed'");
    expectSql(schema, "when v_resolution = 'CONFIRM_REFUNDED' then 'confirmed'::refund_status");
    expectSql(schema, "else 'approved'::refund_status");
    expectSql(schema, "'refund.dispute_resolved'");
    expectSql(schema, "'refund.dispute_reopened'");
    expectSql(schema, "'workflow', 'refund_dispute_resolution'");
    expectSql(schema, "grant execute on function public.resolve_refund_dispute_atomic(uuid, text, text) to authenticated;");
    assert.doesNotMatch(schema, /grant execute on function public\.resolve_refund_dispute_atomic\(uuid, text, text\) to anon/);
  });
